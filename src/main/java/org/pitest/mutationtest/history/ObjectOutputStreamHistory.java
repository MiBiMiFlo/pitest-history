...
public class ObjectOutputStreamHistory implements History {
  ...
  public List<MutationResult> analyse(List<MutationDetails> mutationsForClasses) {
     final MutationAnalyser analyser = new IncrementalAnalyser(
            new CodeHistory(this.code, this.previousResults, this.previousClassPath),
            this.coverageData);
    return analyser.analyse(mutationsForClasses);
  }

  @Override
  public Predicate<ClassName> limitTests(List<MutationDetails> mutants) {
    // Override the new (non-deprecated) signature introduced in commit 5a41bf94.
    // We currently return an unrestricted predicate — incremental filtering
    // happens at analyse() time once coverage data is available. Skipping
    // coverage entirely here would be unsafe because IncrementalAnalyser
    // re-validates killing-test sets against current coverage.
    return c -> true;
  }

  @Override
  public void close() {
    // no-op — the WriterFactory is closed by EntryPoint
  }
